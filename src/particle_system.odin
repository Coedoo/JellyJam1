...
DestroyParticlesystem :: proc(system: ^ParticleSystem) {
...
DrawParticleSystem :: proc(system: ^ParticleSystem) {
    rl.BeginBlendMode(.ADDITIVE)

    #reverse for particle in system.particles {
        c := rl.ColorFromNormalized(particle.color)
        DrawSpriteSize(system.texture, particle.position, c, particle.size, Deg(particle.rotation))
    }
